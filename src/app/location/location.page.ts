...
export class LocationPage implements AfterViewInit {
  ...
  async initializeMap() {
    ...
    this.addMarker(position.coords.latitude, position.coords.longitude, 'Mi ubicación');

    this.searchHospitals(position.coords.latitude, position.coords.longitude);

    this.mapInitialized = true;  // Set the flag to true after initializing the map
  }
  ...
  searchHospitals(lat: number, lng: number) {
    // Utiliza la API de Overpass para buscar hospitales
    const query = `
      [out:json];
      node
        [amenity=hospital]
    ...
    fetch(url)
      ...
      .then(data => {
        ...
        hospitals.forEach((hospital: any) => {
          this.addMarker(hospital.lat, hospital.lon, hospital.tags.name || 'Hospital');
        });
      })
